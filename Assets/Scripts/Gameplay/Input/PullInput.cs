...
public class PullInput : SensorInputBase<PullInput>
{
    ...
    public UnityEvent onPulled = new UnityEvent(); // 监听事件，开始拉动时将执行该事件

    public UnityEvent onReleased = new UnityEvent(); // 监听事件，释放滑块时将执行该事件
    ...
    protected override void Update()
    {
        ...
        if (!_isPulling && NormedValue > pullTolerance)
        { // 开始拉动
            _isPulling = true;
        ...
        if (_isPulling && _lastValues.Max() - NormedValue > releaseTolerance)
        { // 当前拉力 < 前 5 帧的最大拉力 - tolerance，即结束拉动
            _isPulling = false;
